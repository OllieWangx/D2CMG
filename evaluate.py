...
from sklearn.metrics import pairwise, f1_score
from munkres import Munkres
from sklearn import metrics
import numpy as np
import torch
from models.Layers import LogReg


def evaluate(embeds, idx_train, idx_val, idx_test, labels, epoch, lr, iterater=10):
    try:
    ...
    NMI_list, ACC_list = run_kmeans(test_embs, test_lbls, nb_classes)

    st = run_similarity_search(test_embs, test_lbls)

    return np.mean(accs) * 100, np.std(accs) * 100, np.mean(macro_f1s), np.std(macro_f1s), np.mean(micro_f1s), np.std(
        micro_f1s), \
        np.mean(NMI_list), np.std(NMI_list), np.mean(ACC_list), np.std(ACC_list), st
